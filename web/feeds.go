...
func feedItems(txs []types.Transaction) []*feeds.Item {
	...
	for _, tx := range txs {
		...
		if tx.TransactionType == "incoming" {
			description = fmt.Sprintf("Received %s BTC (%s USD) on %s UTC", tx.Value.SatoshisToBTC(true), util.FormatNumber(usdValue), util.ShortUTCTime(tx.BlockTime))
		} else {
			description = fmt.Sprintf("Sent %s BTC (%s USD) on %s UTC", tx.Value.SatoshisToBTC(true), util.FormatNumber(usdValue), util.ShortUTCTime(tx.BlockTime))
		}
		item := &feeds.Item{
			Title:       title,
