...
func (d *SqliteBackend) ItemAddressSummariesBySeries(seriesSlug string) ([]types.ItemAddressSummary, error) {
	query := `SELECT sku, serial, addresses, first_active, redeemed_on, total_value, series_name, series_slug FROM item_address_summary_view WHERE series_slug = ?;`
...
func scanItemAddressSummary(rows *sql.Rows) ([]types.ItemAddressSummary, error) {
	...
	for rows.Next() {
		...
		if firstActive != nil {
			ia.FirstActive, err = time.Parse("2006-01-02 15:04:05-07:00", *firstActive)
			if err != nil {
				return nil, err
			}
		}
		...
		if redeemedOn != nil {
			ia.RedeemedOn, err = time.Parse("2006-01-02 15:04:05-07:00", *redeemedOn)
			if err != nil {
				return nil, err
			}
		}
